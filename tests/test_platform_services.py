...
async def test_service_layer_does_not_import_transport_state() -> None:
    ...
    tree = ast.parse(Path("voiceai/platform/services.py").read_text())
    app_state_access = [
        node
    ...
    assert not app_state_access, "services.py reads app.state"
    assert not store_globals, "services.py touches platform_store"
...
async def test_services_import_errors_via_module_surface() -> None:
    ...
    tree = ast.parse(Path("voiceai/platform/services.py").read_text())
    direct = [node for node in ast.walk(tree) if isinstance(node, ast.ImportFrom) and node.module == "voiceai.errors"]
    assert not direct, "services.py imports voiceai.errors directly (use platform.exceptions)"
