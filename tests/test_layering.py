...
def _layer_files(dotted: str) -> list[pathlib.Path]:
    """Source files for a layer: the module file, or every file in the package."""
    base = REPO_ROOT / dotted.replace(".", "/")
    if base.is_dir():
        return sorted(p for p in base.rglob("*.py") if "__pycache__" not in p.parts)
    return [pathlib.Path(str(base) + ".py")]


def _layer_imports(dotted: str) -> set[str]:
    """Union of top-level imports across a layer's files."""
    imported: set[str] = set()
    for path in _layer_files(dotted):
...
def test_layer_direction_allowed_edges_exist() -> None:
    ...
    services_imports = _layer_imports("voiceai.platform.services")
