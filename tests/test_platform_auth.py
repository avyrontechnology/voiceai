...
Since spec 0006 E4 the legacy `/auth` routes are dark: these contracts drive
the cut-over controller at `/api/v1/auth` (enveloped) mounted on the same
platform app — same statuses, same details, same payloads inside `data`.
"""
...
async def test_login_and_me_roundtrip(ctx):
    await signup_owner(ctx.client)
    me = await ctx.client.get(f"{AUTH}/me")
    assert me.status_code == 200
    assert me.json()["data"]["user"]["email"] == "owner@acme.test"
    assert "*" in me.json()["data"]["scopes"]

    # Wrong password is a flat 401 (no user enumeration).
    bad = await ctx.client.post(f"{AUTH}/login", json={"email": "owner@acme.test", "password": "wrong-password"})
...
async def test_invite_accept_and_role_gates(ctx):
    ...
    invite = await ctx.client.post(f"{AUTH}/invite", json={"email": "member@acme.test", "role": "member"})
    assert invite.status_code == 201, invite.text
    token = invite.json()["data"]["token"]

    async with fresh_client(ctx.app) as member:
        accepted = await member.post(
            f"{AUTH}/accept", json={"token": token, "name": "Member", "password": "correct-horse-2"}
        )
    ...
    async with fresh_client(ctx.app) as other:
        again = await other.post(f"{AUTH}/accept", json={"token": token, "password": "correct-horse-3"})
        assert again.status_code == 400


async def test_non_owner_cannot_invite_admin(ctx):
    await signup_owner(ctx.client)
    invite = await ctx.client.post(f"{AUTH}/invite", json={"email": "admin@acme.test", "role": "admin"})
    token = invite.json()["data"]["token"]
    async with fresh_client(ctx.app) as admin:
        await admin.post(f"{AUTH}/accept", json={"token": token, "password": "correct-horse-6"})
        # Admins may invite members but never owners/admins.
        assert (await admin.post(f"{AUTH}/invite", json={"email": "o2@acme.test", "role": "owner"})).status_code == 403
        ok = await admin.post(f"{AUTH}/invite", json={"email": "m2@acme.test", "role": "member"})
        assert ok.status_code == 201


async def test_viewer_is_read_only(ctx):
    await signup_owner(ctx.client)
    invite = await ctx.client.post(f"{AUTH}/invite", json={"email": "viewer@acme.test", "role": "viewer"})
    token = invite.json()["data"]["token"]
    async with fresh_client(ctx.app) as viewer:
        ...
        assert (await viewer.post(f"{AUTH}/ws-ticket")).status_code == 403
...
async def test_reset_is_owner_only_and_preserves_users(ctx):
    ...
    assert {u["email"] for u in users.json()["data"]["users"]} == {"owner@acme.test", "member@acme.test"}
...
async def test_owner_safety_guards(ctx):
    ...
    invite = await ctx.client.post(f"{AUTH}/invite", json={"email": "owner2@acme.test", "role": "owner"})
    owner2_id = None
    async with fresh_client(ctx.app) as second:
        accepted = await second.post(
            f"{AUTH}/accept", json={"token": invite.json()["data"]["token"], "password": "correct-horse-5"}
        )
        assert accepted.status_code == 201
        owner2_id = accepted.json()["data"]["user"]["user_id"]
    ...
    async with fresh_client(ctx.app) as second:
        await second.post(f"{AUTH}/login", json={"email": "owner2@acme.test", "password": "correct-horse-5"})
...
async def test_logout_kills_session(ctx):
    await signup_owner(ctx.client)
    assert (await ctx.client.get(f"{AUTH}/me")).status_code == 200
    assert (await ctx.client.post(f"{AUTH}/logout")).status_code == 200
    assert (await ctx.client.get(f"{AUTH}/me")).status_code == 401
