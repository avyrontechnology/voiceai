...
class StreamSynthesizer(BaseSynthesizer):
    ...
    async def _wait_for_ws(self, poll_interval: Any = 1) -> None:
        ...
        while not self._is_ws_connected():
            if self.conversation_ended or self.connection_error:
                logger.info(
                    f"Aborting {self.provider_name} sender wait: conversation_ended={self.conversation_ended} connection_error={self.connection_error}"  # noqa: E501 — verbatim legacy line (R8)
                )
                return
    ...
    async def _push_stream(self, message: Any) -> None:
    ...
    def _stamp_turn_start(self, meta_info: Any) -> None:
    ...
    def _compute_first_result_latency(self) -> None:
