...
from voiceai.modules.voice.adapters.tts_runtime import create_ws_data_packet, get_ssl_context
...
class PixaSynthesizer(BaseSynthesizer):
    ...
    def resample_audio(self, audio_bytes: Any) -> Any:
    ...
    async def sender(self, text: Any, sequence_id: Any, end_of_llm_stream: Any = False) -> None:
        ...
        try:
            ...
            if not self.should_synthesize_response(sequence_id):
                logger.info(
                    f"Not synthesizing text as the sequence_id ({sequence_id}) is not in the list of sequence_ids present in the task manager."  # noqa: E501 — verbatim legacy line (R8)
                )
                return
            ...
            while (
                ...
                if self.conversation_ended or self.connection_error:
                    logger.info(
                        f"Aborting pixa sender wait: conversation_ended={self.conversation_ended} connection_error={self.connection_error}"  # noqa: E501 — verbatim legacy line (R8)
                    )
                    return
    ...
    def update_context(self, meta_info: Any) -> None:
        """Update the synthesis context."""
