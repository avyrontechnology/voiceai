...
from voiceai.modules.voice.adapters.tts_runtime import InmemoryScalarCache, create_ws_data_packet
...
class AzureSynthesizer(BaseSynthesizer):
    ...
    def _build_ssml(self, text: Any) -> Any:
        ...
        try:
            while True:
                ...
                def on_synthesizing(evt: Any) -> None:
                    """Handle the provider synthesizing callback."""
                    try:
                        if self.connection_time is None:
                            self.connection_time = round((time.perf_counter() - start_time) * 1000)  # noqa: B023 — verbatim loop-variable capture (R8)
                        asyncio.run_coroutine_threadsafe(chunk_queue.put(evt.result.audio_data), self.loop)  # noqa: B023 — verbatim loop-variable capture (R8)
                    except Exception as e:
                        logger.error(f"Error in synthesizing handler: {e}")

                def on_completed(evt: Any) -> None:
                    """Handle the provider completed callback."""
                ...
                async def _set_done() -> None:
                    done_event.set()  # noqa: B023 — verbatim loop-variable capture (R8)
