...
from voiceai.modules.voice.adapters.tts_runtime import get_ssl_context
...
class CartesiaSynthesizer(StreamSynthesizer):
    ...
    def _update_context(self, meta_info: Any) -> None:
        self.context_id = str(uuid.uuid4())
        self.turn_id = meta_info.get("turn_id", 0)
        self.sequence_id = meta_info.get("sequence_id", 0)
        logger.info(
            f"Cartesia new context_id={self.context_id} turn_id={self.turn_id} sequence_id={self.sequence_id} request_id={self.ws_request_id}"  # noqa: E501 — verbatim legacy line (R8)
        )
    ...
    def form_payload(self, text: Any) -> Any:
        ...
        try:
            ...
            if text != "":
                try:
                    ...
                    logger.info(
                        f"Cartesia sender context_id={self.context_id} text_len={len(text)} request_id={self.ws_request_id}"  # noqa: E501 — verbatim legacy line (R8)
                    )
            ...
            if end_of_llm_stream:
                ...
                try:
                    ...
                    logger.error(
                        f"Error sending end-of-stream signal context_id={self.context_id} request_id={self.ws_request_id}: {e}"  # noqa: E501 — verbatim legacy line (R8)
                    )
                    self.connection_error = str(e)
        ...
        while True:
            try:
                ...
                if "data" in data and data["data"]:
                    ...
                    logger.info(
                        f"No audio data in the response context_id={data.get('context_id')} request_id={self.ws_request_id}"  # noqa: E501 — verbatim legacy line (R8)
                    )
