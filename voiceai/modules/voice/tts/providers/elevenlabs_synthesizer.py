...
from voiceai.modules.voice.adapters.tts_runtime import (
    InmemoryScalarCache,
    get_ssl_context,
    resample,
)
...
class ElevenlabsBase(StreamSynthesizer):
    ...
    def _unpack_receiver_message(self, item: Any) -> Any:
    ...
    def _get_output_format(self) -> Any:
    ...
    async def synthesize_pcm_clip(self, text: Any, sample_rate: Any) -> Any:
        """Render a PCM clip for handoff or prewarm."""
            ...
            try:
                ...
                if "audio" in data and data["audio"] and self.ws_send_time is not None:
                    ...
                    if audio_chunk_count == 1:
                        ...
                        logger.info(
                            f"WS recv FIRST trace_id={self.ws_trace_id} recv_wait={recv_duration:.0f}ms time_since_send={time_since_send:.0f}ms"  # noqa: E501 — verbatim legacy line (R8)
                        )
                    elif recv_duration > 200:
                        gap = (recv_start - last_recv_time) * 1000 if last_recv_time else 0
                        logger.info(
                            f"WS recv SLOW chunk={audio_chunk_count} trace_id={self.ws_trace_id} recv_wait={recv_duration:.0f}ms gap={gap:.0f}ms"  # noqa: E501 — verbatim legacy line (R8)
                        )
                ...
                if "isFinal" in data and data["isFinal"]:
                    ...
                    try:
                        ...
                        logger.info(
                            f"EOS check spoken_chars={len(self.eos_accum_text)} seq_chars={self.current_sequence_chars} enough={spoken_enough}"  # noqa: E501 — verbatim legacy line (R8)
                        )
...
class ElevenlabsV3Synthesizer(ElevenlabsBase):
    ...
    def _classify_lost_socket(self, where: Any) -> None:
    ...
    async def _recycle_socket(self, old_ws: Any) -> None:
    ...
    async def receiver(self) -> None:
        ...
        while True:
            try:
                ...
                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        f"ElevenLabs v3 receiver: {consecutive_errors} consecutive errors, giving up to avoid busy-spin."  # noqa: E501 — verbatim legacy line (R8)
                    )
                    self.connection_error = self.connection_error or str(e)
                    return
                await asyncio.sleep(0.1)
