...
class _FakeOutbound:
    ...
    async def fetch_partner_dids(self, *, talko_api_key: str, talko_api_base_url: str) -> PartnerPreview:
        ...
        return PartnerPreview(
            partner_id=self.fetch_payload.get("partner_id"), dids=list(self.fetch_payload.get("dids") or [])
        )


def _service(
    fake: _FakeOutbound | None = None, base_url: str = "https://talko.test/v1"
...
async def test_connect_needs_explicit_partner_when_empty() -> None:
    ...
    view = await service.connect_partner(payload=ConnectTalkoPartnerRequest(talko_api_key="tkp_live_x", partner_id="9"))
