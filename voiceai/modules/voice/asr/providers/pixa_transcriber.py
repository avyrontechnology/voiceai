    ...
    async def pixa_connect(self, retries: int = 3, timeout: float = 10.0) -> ClientConnection:
        ...
        while attempt < retries:
            try:
                ...
                raise ConnectionError("Timeout while connecting to Pixa websocket")  # noqa: B904 — verbatim raise without cause (R8)
            except InvalidHandshake as e:
                error_msg = str(e)
                if "401" in error_msg or "403" in error_msg:
                    logger.error(f"Pixa authentication failed: Invalid or expired API key - {e}")
                    raise ConnectionError("Pixa authentication failed: Invalid or expired API key")  # noqa: B904 — verbatim raise without cause (R8)
                else:
