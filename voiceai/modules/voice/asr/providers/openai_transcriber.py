...
class OpenAITranscriber(BaseTranscriber):
    ...
    async def _commit_turn(self, ws: ClientConnection) -> None:
    ...
    async def openai_connect(self) -> ClientConnection:
        """Open the OpenAI websocket session."""
        ...
        try:
            ...
            raise ConnectionError("Timeout connecting to OpenAI Realtime")  # noqa: B904 — verbatim raise without cause (R8)
        except InvalidHandshake as e:
            err = str(e)
            if "401" in err or "403" in err:
                raise ConnectionError(f"OpenAI auth failed: {e}")  # noqa: B904 — verbatim raise without cause (R8)
            raise ConnectionError(f"OpenAI handshake failed: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except Exception as e:
            raise ConnectionError(f"OpenAI connection error: {e}")  # noqa: B904 — verbatim raise without cause (R8)
