...
from voiceai.modules.voice.adapters.asr_runtime import create_ws_data_packet, get_ssl_context, timestamp_ms
...
class AssemblyAITranscriber(BaseTranscriber):
    ...
    def get_assemblyai_ws_url(self) -> Any:
    ...
    async def _get_http_transcription(self, audio_data: Any) -> Any:
        ...
        headers = cast("dict[str, str]", {"Authorization": self.api_key})
        ...
        while True:
            async with self.session.get(f"{self.api_url}/{transcript_id}", headers=headers) as response:
                ...
                if result["status"] == "completed":
                    ...
                    http_meta["start_time"] = start_time
                    http_meta["transcriber_latency"] = time.time() - start_time
                    http_meta["transcriber_duration"] = result.get("audio_duration", 0)
                    ...
                    if transcript:
                        ...
                        if turn_is_formatted:
                            ...
                            if self.current_turn_start_time is None:
                                ...
                                meta_info = cast("dict[str, Any]", self.meta_info)
                                if "transcriber_first_result_latency" in meta_info:
                                    del meta_info["transcriber_first_result_latency"]
                            ...
                            try:
                                if self.current_turn_start_time is not None:
                                    ...
                                    stamp_meta = cast("dict[str, Any]", self.meta_info)
                                    stamp_meta["transcriber_total_stream_duration"] = total_stream_duration
                                    stamp_meta["transcriber_latency"] = total_stream_duration
    ...
    async def assemblyai_connect(self) -> Any:
        ...
        try:
            ...
            raise ConnectionError("Timeout while connecting to AssemblyAI websocket")  # noqa: B904 — verbatim raise without cause (R8)
        except InvalidHandshake as e:
            logger.error(f"Invalid handshake during AssemblyAI websocket connection: {e}")
            raise ConnectionError(f"Invalid handshake during AssemblyAI websocket connection: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except ConnectionClosedError as e:
            logger.error(f"AssemblyAI websocket connection closed unexpectedly: {e}")
            raise ConnectionError(f"AssemblyAI websocket connection closed unexpectedly: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except Exception as e:
            logger.error(f"Unexpected error connecting to AssemblyAI websocket: {e}")
            raise ConnectionError(f"Unexpected error connecting to AssemblyAI websocket: {e}")  # noqa: B904 — verbatim raise without cause (R8)
