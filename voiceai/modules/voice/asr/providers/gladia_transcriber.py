...
class GladiaTranscriber(BaseTranscriber):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.gladia_session_id: str | None = None
    ...
    async def gladia_connect(self, retries: int = 3, timeout: float = 10.0) -> ClientConnection:
        ...
        while attempt < retries:
            try:
                ...
                raise ConnectionError("Timeout while connecting to Gladia WebSocket")  # noqa: B904 — verbatim raise without cause (R8)
            except InvalidHandshake as e:
                error_msg = str(e)
                if "401" in error_msg or "403" in error_msg:
                    logger.error(f"Gladia authentication failed: {e}")
                    raise ConnectionError(f"Gladia authentication failed: Invalid API key - {e}")  # noqa: B904 — verbatim raise without cause (R8)
                else:
    ...
    async def _close_gladia(self, ws: ClientConnection) -> None:
