...
class ElevenLabsTranscriber(BaseTranscriber):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # ElevenLabs valid ranges: vad_threshold (0.1-0.9), min_speech_duration_ms (50-2000), min_silence_duration_ms (50-2000)  # noqa: E501
        ...
        async for msg in ws:
            try:
                ...
                if msg_type == "session_started":
                    ...
                    # Skip when include_timestamps is enabled - we'll process committed_transcript_with_timestamps instead  # noqa: E501
                    # ElevenLabs sends both messages for the same utterance despite docs saying they're mutually exclusive  # noqa: E501
    ...
    async def elevenlabs_connect(self) -> Any:
        ...
        try:
            ...
            raise ConnectionError("Timeout while connecting to ElevenLabs websocket")  # noqa: B904 — verbatim raise without cause (R8)
        except InvalidHandshake as e:
            logger.error(f"Invalid handshake during ElevenLabs websocket connection: {e}")
            raise ConnectionError(f"Invalid handshake during ElevenLabs websocket connection: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except ConnectionClosedError as e:
            logger.error(f"ElevenLabs websocket connection closed unexpectedly: {e}")
            raise ConnectionError(f"ElevenLabs websocket connection closed unexpectedly: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except Exception as e:
            logger.error(f"Unexpected error connecting to ElevenLabs websocket: {e}")
            raise ConnectionError(f"Unexpected error connecting to ElevenLabs websocket: {e}")  # noqa: B904 — verbatim raise without cause (R8)
