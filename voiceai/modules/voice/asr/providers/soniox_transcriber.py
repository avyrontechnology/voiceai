...
class SonioxTranscriber(BaseTranscriber):
    ...
    def get_soniox_ws_url(self) -> Any:
        """Return the Soniox websocket URL."""
    ...
    async def soniox_connect(self) -> Any:
        ...
        try:
            ...
            raise ConnectionError("Timeout while connecting to Soniox websocket")  # noqa: B904 — verbatim raise without cause (R8)
        except InvalidHandshake as e:
            raise ConnectionError(f"Invalid handshake during Soniox websocket connection: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except ConnectionClosedError as e:
            raise ConnectionError(f"Soniox websocket connection closed unexpectedly: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except Exception as e:
            raise ConnectionError(f"Unexpected error connecting to Soniox websocket: {e}")  # noqa: B904 — verbatim raise without cause (R8)
    ...
    def _mark_last_interim_final(self) -> None:
    ...
    def _build_finalized_turn_latency(self, final_transcript: Any) -> None:
    ...
    def _find_audio_send_timestamp(self, audio_position_s: Any) -> Any:
