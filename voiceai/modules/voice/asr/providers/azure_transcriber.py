...
class AzureTranscriber(BaseTranscriber):
    ...
    def _check_and_process_end_of_stream(self, ws_data_packet: Any) -> Any:
    ...
    async def send_audio_to_transcriber(self) -> None:
        """Send audio to the transcriber."""
        try:
            while True:
    ...
    async def recognized_handler(self, evt: Any) -> None:
        """Handle the provider recognized callback."""
    ...
    async def canceled_handler(self, evt: Any) -> None:
        """Handle the provider canceled callback."""
    ...
    async def session_started_handler(self, evt: Any) -> None:
        """Handle the provider session-started callback."""
        logger.info(f"Session start event received: {evt} | run_id - {self.run_id}")
        self.start_time = time.time()

    async def session_stopped_handler(self, evt: Any) -> None:
        """Handle the provider session-stopped callback."""
