...
from voiceai.modules.voice.adapters.asr_runtime import create_ws_data_packet, resample, timestamp_ms, ulaw_to_pcm
...
class GeminiTranscriber(BaseTranscriber):
    ...
    def _resolve_language_codes(self) -> Any:
    ...
    async def gemini_connect(self) -> Any:
        ...
        try:
            ...
            raise ConnectionError("Timeout while connecting to Gemini Live websocket")  # noqa: B904 — verbatim raise without cause (R8)
        except InvalidHandshake as e:
            raise ConnectionError(f"Invalid handshake during Gemini Live websocket connection: {e}")  # noqa: B904 — verbatim raise without cause (R8)
        except ConnectionClosed as e:
            raise ConnectionError(f"Gemini Live websocket closed during setup: {e}")  # noqa: B904 — verbatim raise without cause (R8)

    def _to_gemini_pcm(self, data: Any) -> Any:
    ...
    async def sender_stream(self, ws: Any) -> None:
    ...
    def _finalize_turn_latency(self, final_transcript: Any) -> None:
    ...
    def _flush_pending_final(self) -> Any:
