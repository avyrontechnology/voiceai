...
class SmallestTranscriber(BaseTranscriber):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.smallest_session_id: str | None = None
    ...
    def get_smallest_ws_url(self) -> str:
        ...
        logger.info(
            f"Smallest WebSocket URL params - language: {self.language}, sample_rate: {self.sampling_rate}, encoding: {self.encoding}, word_timestamps: {self.word_timestamps}"  # noqa: E501 — verbatim legacy line (R8)
        )
    ...
    async def smallest_connect(self, retries: int = 3, timeout: float = 10.0) -> ClientConnection:
        ...
        while attempt < retries:
            try:
                ...
                raise ConnectionError("Timeout while connecting to Smallest AI WebSocket")  # noqa: B904 — verbatim raise without cause (R8)
            except InvalidHandshake as e:
                error_msg = str(e)
                if "401" in error_msg or "403" in error_msg:
                    logger.error(f"Smallest AI authentication failed: {e}")
                    raise ConnectionError(f"Smallest AI authentication failed: Invalid API key - {e}")  # noqa: B904 — verbatim raise without cause (R8)
                else:
                ...
                raise ConnectionError(f"Smallest AI WebSocket connection closed unexpectedly: {e}")  # noqa: B904 — verbatim raise without cause (R8)
    ...
    async def _force_finalize_utterance(self) -> None:
    ...
    async def monitor_utterance_timeout(self) -> None:
    ...
    async def _close_smallest(self, ws: ClientConnection) -> None:
    ...
    def _find_audio_send_timestamp(self, audio_position: float) -> Any:
        """
    ...
    async def receiver(self, ws: ClientConnection) -> None:
        ...
        async for msg in ws:
            try:
                ...
                full_transcript = data.get("full_transcript", "").strip()  # noqa: F841 — verbatim dead local (R8)
