...
class SarvamTranscriber(BaseTranscriber):
    ...
    def _convert_audio_to_wav(self, audio_data: Any) -> bytes | None:
        try:
    ...
    async def sarvam_connect(self, retries: int = 3, timeout: float = 10.0) -> ClientConnection:
        ...
        while attempt < retries:
            try:
                ...
                raise ConnectionError("Timeout while connecting to Sarvam websocket")  # noqa: B904 — verbatim raise without cause (R8)
            except InvalidHandshake as e:
                error_msg = str(e)
                if "401" in error_msg or "403" in error_msg:
                    logger.error(f"Sarvam authentication failed: Invalid or expired API key - {e}")
                    raise ConnectionError(f"Sarvam authentication failed: Invalid or expired API key - {e}")  # noqa: B904 — verbatim raise without cause (R8)
                elif "404" in error_msg:
                    logger.error(f"Sarvam endpoint not found - check model/configuration: {e}")
                    raise ConnectionError(f"Sarvam endpoint not found: {e}")  # noqa: B904 — verbatim raise without cause (R8)
                else:
    ...
    async def send_heartbeat(self, ws: ClientConnection, interval_sec: float = 10.0) -> None:
