...
from voiceai.modules.voice.adapters.asr_runtime import REGEN_SETTLE_EXCLUDED_TRANSCRIBERS, LIDProvider
...
class TranscriberPool:
    ...
    def __init__(
        ...
        self._lid_pending_lang: str | None = None
    ...
    def is_active_transcriber_alive(self) -> Any:
    ...
    @staticmethod
    def _silence_frame(encoding: Any) -> Any:
    ...
    async def _standby_keepalive(self) -> None:
    ...
    async def _handle_lid_signal(self, lang: str, confidence: float) -> None:
        ...
        if confidence is not None and confidence < self._LID_CONFIDENCE_THRESHOLD:
            logger.debug(
                f"TranscriberPool LID: {lang} conf={f'{confidence:.2f}' if confidence is not None else 'n/a'} below threshold — suppressed"  # noqa: E501 — verbatim legacy line (R8)
            )
        ...
        logger.info(
            f"TranscriberPool LID: {lang} conf={f'{confidence:.2f}' if confidence is not None else 'n/a'} (provider={self._lid_provider_name})"  # noqa: E501 — verbatim legacy line (R8)
        )
        ...
        if self._lid_mode == "shadow":
            logger.info(
                ...
                f"(lang={lang}, conf={f'{confidence:.2f}' if confidence is not None else 'n/a'}, suppressed_reason=shadow_mode)"  # noqa: E501 — verbatim legacy line (R8)
            )
    ...
    def take_lid_transcript(self) -> Any:
    ...
    def lid_buffer_language(self) -> Any:
    ...
    def lid_buffer_language_confidence(self) -> Any:
    ...
    def lid_buffer_segments(self) -> Any:
    ...
    async def switch(self, label: Any) -> None:
    ...
    def get_active_transcriber_info(self) -> Any:
    ...
    def _record_detector_health(self) -> None:
