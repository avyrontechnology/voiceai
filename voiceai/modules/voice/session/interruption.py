"""Interruption state: turn-taking gates, sequence IDs and speaking stats (spec 0004, B10).

The ``InterruptionManager`` class moved here VERBATIM from
``voiceai/agent_manager/interruption_manager.py`` (512 lines): the audio-gate
(``get_audio_send_status``), the interruption gates (``should_trigger_interruption`` /
``is_false_interruption``), the speech/interruption lifecycle callbacks, the
sequence-ID management (``invalidate`` / ``revalidate`` / ``get_next`` / ``retire``),
the incremental-delay logic and the state queries. The B5-B9 seams apply unchanged:

* **No facade needed.** The class is self-contained (only ``time`` + the call-wide
  tunables passed through its constructor); it imports no legacy engine code, so
  ``§3.1 bridge 3`` is the constructor call site in ``TaskManager.__init__``,
  which keeps constructing it unchanged until composition (B13a).
* **Old path stays a shim.** ``voiceai/agent_manager/interruption_manager.py``
  becomes a pure ``# legacy-shim(spec-0004)`` re-export of this class, so the four
  legacy-tree importers (``task_manager``, ``test_interruption_gate``,
  ``test_characterization_task_manager_construction`` and friends) keep resolving
  by identity.
* **This module is the lookup site for nothing new** (R3): the class logs through
  ``otobaai`` (rule 3; log content preserved) and otherwise touches only stdlib.

Mechanical accommodations (reported): ``configure_logger`` → ``otobaai`` voice
logger, signatures gained complete annotations (rule 6), public methods gained
...
class InterruptionManager:
    ...
    def __init__(
        ...
        self._open_event: dict | None = None  # latest event awaiting user_end_s
        ...
        self._last_sent_sequence_id: int | None = None  # dedup guard for on_agent_speech_started
        self.longest_agent_monologue_ms: float = 0.0

        self._adjusted_user_stop_ts: float | None = None
    ...
    def on_user_speech_ended(
        ...
        user_stop_ts_wall: float | None = None,
    ) -> None:
    ...
    def on_interruption_triggered(self, asr_turn_id: int | None = None) -> None:
        ...
        logger.info(
            f"Interruption triggered — turn_id={self.turn_id}, user_interrupted_agent_count={self.user_interrupted_agent_count}"  # noqa: E501 — verbatim legacy log line (R8)
        )

    def on_agent_interrupted_user(self, asr_turn_id: int | None = None) -> None:
    ...
    def record_interrupted_transcriber_turn(self, turn_id: object) -> None:
    ...
    def on_successful_response_delivered(self, sequence_id: int | None = None) -> None:
    ...
    def revalidate_sequence_id(self, sequence_id: int) -> None:
        """Re-adds a sequence ID after invalidation when a new response's sequence_id was already allocated."""
    ...
    def retire_sequence_id(self, sequence_id: int | None) -> None:
    ...
    def has_pending_responses_excluding(self, sequence_id: int | None) -> bool:
