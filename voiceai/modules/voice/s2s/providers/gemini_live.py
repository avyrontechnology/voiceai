"""Gemini Live speech-to-speech provider (spec 0004, step B5).

Moved verbatim from ``voiceai/s2s/gemini_live_s2s.py`` (now a
spec-0004-tagged legacy shim re-exporting this module). Mechanical accommodations
only, bodies untouched: this docstring, the ``otobaai`` logger (AGENTS.md rule 3),
``clean_gemini_schema`` riding the §3.1 adapter bridge instead of a direct legacy
import, PEP 604 / built-in generics in signatures, an explicit ``Any`` for the
version-portable websocket handle, docstrings on the public methods, and a ``noqa``
tag on the verbatim swallow-and-continue ``except`` (preserved quirk; R8 owns fixes).
"""
...
from voiceai.modules.voice.adapters.s2s_runtime import clean_gemini_schema
...
from voiceai.modules.voice.s2s.events import (
    AudioDelta,
    FunctionCall,
...
class GeminiLiveS2S(BaseS2SProvider):
    ...
    def __init__(
        ...
        **kwargs: Any,  # why: the legacy constructor kwargs contract is a free-form seam
        ...
        self._ws: Any = None  # why: the websockets client protocol, version-portable
        self._closed = False
        self._resumption_handle: str | None = None
        self._reconnecting = False
        self._pending_tool_results: list = []
        self._turn_usage: S2SUsage | None = None
    ...
    async def receive_events(self) -> AsyncGenerator:
        """Yield provider-agnostic events until the session ends, resuming on drops."""
    ...
    async def _resume_session(self) -> None:
        ...
        try:
            if self._ws:
                ...
                except Exception:  # noqa: S110 — verbatim quirk: a dead socket's close is best-effort
                    pass
            await self._open_session()
    ...
    async def send_dtmf(self, digits: str) -> None:
        """Forward telephony keypad digits to the model as a text turn."""
        await self._send(
            {
                "clientContent": {
...
def _duration_to_ms(duration: Any) -> int | None:  # why: protobuf duration arrives as str/number/None
