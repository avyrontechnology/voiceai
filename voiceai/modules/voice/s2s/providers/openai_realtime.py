"""OpenAI Realtime speech-to-speech provider (spec 0004, step B5).

Moved verbatim from ``voiceai/s2s/openai_realtime_s2s.py`` (now a
...
PEP 604 / built-in generics in signatures, an explicit ``Any`` for the version-portable
websocket handle, docstrings on the public methods, and ``noqa`` tags on the two
verbatim swallow-and-continue ``except`` blocks (preserved quirks; R8 owns real fixes).
"""
...
class OpenAIRealtimeS2S(BaseS2SProvider):
    ...
    def is_reasoning_model(self) -> bool:
        """Whether the configured model accepts ``reasoning.effort``."""
    ...
    async def receive_events(self) -> AsyncGenerator:
        """Yield provider-agnostic events until the session ends, reconnecting on drops."""
    ...
    async def send_text(self, text: str) -> None:
        ...
        except Exception:  # noqa: S110 — verbatim quirk: the cancel is best-effort by design
            pass
        await self._send(
            {
                "type": "conversation.item.create",
    ...
    async def send_dtmf(self, digits: str) -> None:
        """Forward telephony keypad digits to the model as a text item."""
    ...
    def _extract_usage(self, event: dict) -> S2SUsage | None:
