...
from voiceai.modules.voice.adapters.io_runtime import AUDIO_STREAM_END_SENTINELS, calculate_audio_duration
...
class TelephonyOutputHandler(DefaultOutputHandler):
    ...
    async def handle(self, ws_data_packet: Any) -> None:
            ...
            try:
                ...
                if audio_chunk and self.stream_sid and len(audio_chunk) != 1:
                    if audio_chunk != b"\x00\x00":
                        ...
                        logger.info(
                            "VOICEAI_TRACE_TEL send_pre_mark mark_id=%s seq=%s turn=%s response_uid=%s group_uid=%s category=%s",  # noqa: E501 — verbatim legacy line (R8)
                            mark_id,
                    ...
                    logger.info(
                        "VOICEAI_TRACE_TEL send_post_mark mark_id=%s seq=%s turn=%s response_uid=%s group_uid=%s final=%s category=%s text_len=%s",  # noqa: E501 — verbatim legacy line (R8)
                        mark_id,
                        meta_info.get("sequence_id"),
                        meta_info.get("turn_id"),
                        meta_info.get("response_uid"),
