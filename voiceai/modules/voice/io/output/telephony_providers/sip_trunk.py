...
class SipTrunkOutputHandler(TelephonyOutputHandler):
    ...
    def _get_output_config(self) -> Any:
        try:
    ...
    def _schedule_finish(self, generation: int) -> None:
    ...
    async def _settle_and_finish(self, generation: int, delay: float) -> None:
    ...
    async def flush_media(self) -> None:
        """Flush Asterisk's media buffer."""
        await self._send_control("FLUSH_MEDIA")

    async def _flush_buffer(self, generation: int) -> None:
    ...
    async def send_hangup(self) -> None:
        """Send the HANGUP control to Asterisk."""
