...
from voiceai.modules.voice.adapters.io_runtime import IS_USER_ONLINE_MESSAGE, create_ws_data_packet
...
class DefaultInputHandler:
    ...
    def __init__(
        ...
        self.last_final_chunk_sequence_id: int | None = None
        self.last_final_chunk_played_ts: float | None = None

    def get_calculated_plivo_latency(self) -> Any:
        """Return the smoothed Plivo latency estimate."""
        return self.calculated_plivo_latency

    def _calculate_and_update_latency(self, mark_event_meta_data_obj: Any) -> None:
    ...
    def get_audio_chunks_received(self) -> Any:
        """Return and reset the received audio chunk count."""
        audio_chunks_received = self.audio_chunks_received
        self.audio_chunks_received = 0
        return audio_chunks_received

    def update_is_audio_being_played(self, value: Any) -> None:
        """Flag whether audio is reaching the user."""
    ...
    def is_audio_being_played_to_user(self) -> Any:
        """Whether audio is currently reaching the user."""
        return self._is_audio_being_played_to_user

    def get_response_heard_by_user(self) -> Any:
        """Return and clear the heard-text accumulator."""
        response = self.response_heard_by_user
        self.response_heard_by_user = ""
        return response.strip()

    def reset_response_heard_by_user(self) -> None:
        """Clear the heard-text accumulators."""
        self.response_heard_by_user = ""
    ...
    async def stop_handler(self) -> None:
        """Stop the handler and release its task."""
        self.running = False
        try:
            if not self.queue:
    ...
    def get_current_mark_started_time(self) -> Any:
        """Return when the current mark started playing."""
        return self.update_start_ts

    def welcome_message_played(self) -> Any:
        """Whether the welcome message has played."""
    ...
    def process_mark_message(self, packet: Any) -> None:
        """Record a provider mark-ack in the ledger."""
        ...
        logger.info(
            "VOICEAI_TRACE_ACK applied mark_id=%s type=%s seq=%s turn=%s response_uid=%s final=%s heard_len=%s last_heard_turn=%s last_heard_response_uid=%s",  # noqa: E501 — verbatim legacy line (R8)
            packet.get("name"),
            message_type,
    ...
    def __process_mark_event(self, packet: Any) -> None:
        self.process_mark_message(packet)

    def __process_audio(self, audio: Any) -> None:
    ...
    def __process_text(self, text: Any) -> None:
    ...
    async def _listen(self) -> None:
        ...
        except WebSocketDisconnect as e:  # noqa: F841 — verbatim unused binding (R8)
