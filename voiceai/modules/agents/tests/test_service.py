...
class FakePromptStore:
    ...
    async def delete_prompts(self, agent_id: str) -> bool:
        """Remove the payload; `True` when a key existed."""
...
async def test_delete_removes_the_definition_and_its_prompts():
    """T3 ends the orphan quirk: DELETE removes the definition and its payload."""
    store = FakeDefinitionStore({AGENT_ID: stored_record()})
    prompt_store = FakePromptStore({AGENT_ID: PROMPTS})
    service = build_service(store, prompt_store)
    ...
    assert AGENT_ID not in prompt_store.stored
