"""Realtime call orchestration (TaskManager).

Architecture note (2026-09-15): this module is the permanent stateful
orchestration home for a call. Cohesive subsystems live in siblings
(S2SMixin, interruption_manager, voicemail_handler, utils, static_methods),
but the TaskManager class itself is NOT split further: its ~30 dunder-private
methods are name-mangled, and doubles/tests bind those mangled names, so a
class split would churn the whole suite for no runtime gain. Revisit only
with a de-mangling pass first.
"""
...
from .s2s_mixin import S2SMixin
...
from voiceai.core.resilience import LoopFailure, TaskRegistry, call_soft, iteration_guard, log_ignored, with_timeout
...
from voiceai.core.environment import get_str, set_env
from voiceai.otobaai_logger import get_logger
from .constants import (
    BACKCHANNELING_PRESETS_DIR_ENV,
    CALL_TRANSFER_WEBHOOK_URL_ENV,
    CHECK_FOR_COMPLETION_LLM_ENV,
    DEFAULT_LANGUAGE_SWITCH_AUDIO_GAP_S,
    DEFAULT_LANGUAGE_SWITCH_CORROBORATED_MIN_CONFIDENCE,
    DEFAULT_LANGUAGE_SWITCH_DECIDE_TIMEOUT_S,
    DEFAULT_LANGUAGE_SWITCH_DETECTOR_MIN_PROB,
    DEFAULT_LANGUAGE_SWITCH_IDLE_FLUSH_S,
    DEFAULT_LANGUAGE_SWITCH_MAX_HOLD_S,
    DEFAULT_LANGUAGE_SWITCH_MIN_CONFIDENCE,
    DEFAULT_LANGUAGE_SWITCH_MIN_SEGMENT_AUDIO_S,
    DEFAULT_LANGUAGE_SWITCH_MISMATCH_IDLE_FLUSH_S,
    DEFAULT_LANGUAGE_SWITCH_SETTLE_MS,
    DEFAULT_LID_PROVIDER,
    DEFAULT_RAG_SERVER_URL,
    DEFAULT_S2S_ACK_INDEPENDENT,
    DEFAULT_S2S_TALKO_ENCODING,
    GEMINI_API_KEY_ENV,
    GOOGLE_API_KEY_ENV,
    LANGUAGE_SWITCH_AUDIO_GAP_S_ENV,
    LANGUAGE_SWITCH_CORROBORATED_MIN_CONFIDENCE_ENV,
    LANGUAGE_SWITCH_DECIDE_TIMEOUT_S_ENV,
    LANGUAGE_SWITCH_DETECTOR_MIN_PROB_ENV,
    LANGUAGE_SWITCH_EXPLICIT_MIN_CONFIDENCE_ENV,
    LANGUAGE_SWITCH_IDLE_FLUSH_S_ENV,
    LANGUAGE_SWITCH_MAX_HOLD_S_ENV,
    LANGUAGE_SWITCH_MIN_CONFIDENCE_ENV,
    LANGUAGE_SWITCH_MIN_SEGMENT_AUDIO_S_ENV,
    LANGUAGE_SWITCH_MISMATCH_IDLE_FLUSH_S_ENV,
    LANGUAGE_SWITCH_SETTLE_MS_ENV,
    LID_PROVIDER_ENV,
    OPENAI_API_KEY_ENV,
    PRE_CALL_WEBHOOK_DISPATCH_URL_ENV,
    RAG_SERVER_URL_ENV,
...
from .static_methods import build_lid_decision_record
from .static_methods import inject_end_call_tool as _inject_end_call_tool
from .utils import (
    HANDOFF_CLIP_CACHE,
    HANDOFF_CLIP_CACHE_MAX,
    NON_NODE_RESPONSE_CATEGORIES as _NON_NODE_RESPONSE_CATEGORIES,
    asr_id_to_int,
    is_alphanumeric_readout,
    s2s_ack_independent as _s2s_ack_independent,
    s2s_talko_encoding as _s2s_talko_encoding,
    trailing_utterance_text,
    welcome_pcm_upsampled,
)


class TaskManager(S2SMixin, BaseManager):
        ...
        **kwargs,
        ...
        if task_id == 0:
            ...
            if self.conversation_config is not None:
                ...
                self.check_for_completion_llm = get_str(CHECK_FOR_COMPLETION_LLM_ENV)
                ...
                if self.should_backchannel and not turn_based_conversation and task_id == 0:
                    ...
                    self.backchanneling_audios = f"{kwargs.get('backchanneling_audio_location', get_str(BACKCHANNELING_PRESETS_DIR_ENV))}/{self.synthesizer_voice.lower()}"
    ...
    def fire_pre_call_webhook(self, webhook_url, called_fun, resp, meta_info, webhook_param=None):
        ...
        dispatch_url = get_str(PRE_CALL_WEBHOOK_DISPATCH_URL_ENV)
        if dispatch_url:
        ...
        try:
            if self.task_config["tools_config"]["transcriber"] is not None:
                ...
                if "multilingual" in transcriber_config and transcriber_config.get("multilingual"):
                    ...
                    ) or get_str(LID_PROVIDER_ENV, DEFAULT_LID_PROVIDER)
    ...
    def __get_agent_object(self, llm, agent_type, assistant_config=None):
        ...
        if agent_type == "simple_llm_agent":
            ...
            rag_server_url = self.kwargs.get("rag_server_url", get_str(RAG_SERVER_URL_ENV, DEFAULT_RAG_SERVER_URL))
            ...
            set_env("RAG_SERVER_URL", rag_server_url)
    ...
    async def _execute_transfer_call_webhook(self, called_fun, url, param, resp, meta_info):
        ...
        if url is None:
            url = get_str(CALL_TRANSFER_WEBHOOK_URL_ENV)
    ...
    def __arm_lid_playback_gate(self, sequence_id, decision_task) -> None:
        ...
        self.lid_playback_gate = {
            ...
            + float(get_str(LANGUAGE_SWITCH_MAX_HOLD_S_ENV, DEFAULT_LANGUAGE_SWITCH_MAX_HOLD_S)),
        }
    ...
    def __detector_corroborates(segments, target) -> bool:
        ...
        min_prob = float(get_str(LANGUAGE_SWITCH_DETECTOR_MIN_PROB_ENV, DEFAULT_LANGUAGE_SWITCH_DETECTOR_MIN_PROB))
        min_segment_s = float(
            get_str(LANGUAGE_SWITCH_MIN_SEGMENT_AUDIO_S_ENV, DEFAULT_LANGUAGE_SWITCH_MIN_SEGMENT_AUDIO_S)
        )
    ...
    def __switch_decide_timeout_s(self) -> float:
        ...
        return float(get_str(LANGUAGE_SWITCH_DECIDE_TIMEOUT_S_ENV, DEFAULT_LANGUAGE_SWITCH_DECIDE_TIMEOUT_S))

    def __switch_settle_ms(self) -> int:
        """Detector-tail settle before draining its buffer. Shared with the hold budget."""
        return int(get_str(LANGUAGE_SWITCH_SETTLE_MS_ENV, DEFAULT_LANGUAGE_SWITCH_SETTLE_MS))
    ...
    def __switch_audio_gap_s(self) -> float:
        ...
        return float(get_str(LANGUAGE_SWITCH_AUDIO_GAP_S_ENV, DEFAULT_LANGUAGE_SWITCH_AUDIO_GAP_S))
    ...
    async def __lid_idle_watcher(self):
        ...
        """
        idle_flush_s = float(get_str(LANGUAGE_SWITCH_IDLE_FLUSH_S_ENV, DEFAULT_LANGUAGE_SWITCH_IDLE_FLUSH_S))
        ...
        mismatch_idle_flush_s = float(
            get_str(LANGUAGE_SWITCH_MISMATCH_IDLE_FLUSH_S_ENV, DEFAULT_LANGUAGE_SWITCH_MISMATCH_IDLE_FLUSH_S)
        )
        try:
    ...
    async def __run_language_switch(
        ...
        min_conf = float(get_str(LANGUAGE_SWITCH_MIN_CONFIDENCE_ENV, DEFAULT_LANGUAGE_SWITCH_MIN_CONFIDENCE))
        ...
        if self.language_switcher.explicit_only:
            ...
            explicit_min_conf = float(get_str(LANGUAGE_SWITCH_EXPLICIT_MIN_CONFIDENCE_ENV, str(min_conf)))
    ...
    async def _synthesize(self, message):
        ...
        # S2S/text agents deliberately carry `"synthesizer": None` (the realtime
        # model or the dashboard speaks instead): skip the classic TTS path with
        # a clear log instead of crashing on the unset provider attribute
        # mid-call. A *missing* key is a misconfiguration and keeps the old
        # loud failure path below.
        if self.task_config.get("tools_config", {}).get("synthesizer", "present") is None:
            logger.info("no synthesizer configured; skipping classic TTS path")
            self.response_in_pipeline = False
            self._synthesis_awaiting_first_audio = False
            self._turn_audio_flushed.set()
            return
